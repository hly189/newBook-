...
void Helpers::corretingDateInput(int &day) {
	while (day < 1 || day > 31) {
		std::cout << "Invalid Day Input. Day must be between 1-31. Please enter again: " << std::endl;
		std::cin >> day;
	}
}

void Helpers::corretingMonthInput(int &month) {
	while (month < 1 || month > 12) {
		std::cout << "Invalid Month Input. Day must be between 1-12. Please enter again: " << std::endl;
		std::cin >> month;
	}
}

void Helpers::corretingYearInput(int &year) {
	if (year >= 1300) return;
	while (year < 1300) {
		std::cout << "Invalid Year Input. Day must be greater than 1390.Please enter again: " << std::endl;
		std::cin >> year;
	}
}