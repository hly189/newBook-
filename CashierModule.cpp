...
void CashierModule::showCustomeReceiptScreen(int startingPoint, double saleTax) {
	...
	for (int i = startingPoint; i < size; i++) {
		BookSoldInfo tempBook = bookSoldArray[i];
...
void CashierModule::cashierFunction() {
	...
	std::cout << "Day Sold (dd)";
	Helpers::correctingValidInput(daySold);
	Helpers::corretingDateInput(daySold);
	cin.ignore();
	std::cout << "Month Sold (mm) ";
	Helpers::correctingValidInput(monthSold);
	Helpers::corretingMonthInput(monthSold);
	cin.ignore();
	std::cout << "Year Sold (yyyy) ";
	Helpers::correctingValidInput(yearSold);
	Helpers::corretingYearInput(yearSold);
	cin.ignore(); 
	do {

		// Asking for day, month and year  sold and validating them
		...
		if (quantitySold > bookSold->getQuantityOnHand()) {
			...
			BookSoldInfo bookSoldObject = BookSoldInfo(saleTax, totalPrice, quantitySold, newRemaining, daySold, monthSold, yearSold);
			string title = bookSold->getTitle();
			string isbn = bookSold->getIsbn();
			double retail = bookSold->getRetailPrice(); 
			bookSoldObject.setTitle(title);
			bookSoldObject.setIsbn(bookSold->getIsbn()); 
			bookSoldObject.setRetail(retail); 
		...
		while (toupper(confirm) != 'Y' && toupper(confirm) != 'N')
		{
			std::cout << "Invalid entry. Please enter 'y/Y' to confirm or 'n/N' to continue ";
			std::cin >> confirm;
		}
